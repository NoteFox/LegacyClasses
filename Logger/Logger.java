 ...
 */
public class Logger extends Thread implements LoggerInterface {
     ...
     */
    public Logger(String logDefaultDir, int maxLogFileSize, boolean logTerminalOutput) throws IOException {
        if (!new File(logDefaultDir).exists()) {
            boolean result = new File(logDefaultDir).mkdirs();
            if (!result) {
